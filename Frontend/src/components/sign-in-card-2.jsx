...
// A simple utility to conditionally join class names, replacing the `cn` function from the original code.
...
export default function App() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // The "Failed to fetch" error is often caused by a CORS policy issue when the
      // frontend is on a different origin than the backend. The backend server
      // (http://localhost:3000) needs to be configured to allow requests from
      // this origin.
      console.error('Login error:', error);
      if (error instanceof TypeError && error.message.includes('Failed to fetch')) {
        console.error('Potential CORS issue: The browser blocked the request. Ensure your backend server at http://localhost:3000 has CORS enabled and is configured to accept requests from this origin.');
        alert('An error occurred while connecting to the server. This may be a network issue or a server configuration problem.');
      } else {
        alert('An unexpected error occurred. Please try again later.'); // Use a custom modal in a real app
      }
    } finally {
      setIsLoading(false);
    }
